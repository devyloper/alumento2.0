	...
	margin:unset;
        ...
        <header class="masthead">
            ...
            <div class="container-fluid mt-3 pt-2">
                ...
                $s_type=htmlentities($_GET['type']) ;
                if($s_type==="All")
                {
                    $alumni = $conn->query("SELECT a.*,c.course,Concat(a.firstname,' ',a.lastname) as name from alumnus_bio a inner join courses c on c.id = a.course_id where a.status=1 order by Concat(a.firstname,' ',a.lastname) asc");
                }
                else{
                    $alumni = $conn->query("SELECT a.*,c.course,Concat(a.firstname,' ',a.lastname) as name from alumnus_bio a inner join courses c on c.id = a.course_id where a.status=1 and a.companysector like '". $s_type ."'  order by Concat(a.firstname,' ',a.lastname) asc");
                }
                ...
                <div class="card alumni-list" data-id="<?php echo $row['id'] ?>">
                    ...
                    <div class="card-body">
                        <div class="row align-items-center h-100">
                            <div class="">
                                <div>
                                <p class="filter-txt"><b><a href ="<?php echo $row['linkedinid'] ?>" style= "color:black" target= "_top"><?php echo $row['name'] ?></a></b></p>
                                <hr class="divider w-100" style="max-width: calc(100%)">
                                <!-- <p class="filter-txt">Email: <b><?php echo $row['email'] ?></b></p> -->
                                <p class="filter-txt">Native Place: <b><?php echo $row['native'] ?></b></p>
                                <p class="filter-txt">Course: <b><?php echo $row['course'] ?></b></p>
                                <p class="filter-txt">Batch: <b><?php echo $row['batch'] ?></b></p>
                                <p class="filter-txt">Company Sector: <b><?php echo $row['companysector'] ?></b></p>
                                <p class="filter-txt">Currently working in <b><?php echo $row['connected_to'] ?></b></p>
